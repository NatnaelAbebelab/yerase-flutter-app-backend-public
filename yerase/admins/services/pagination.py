...
class AdminsDataPagination(Pagination):
    
    def paginate_admins(self, request, queryset):
        paginated_queryset = self.paginate_queryset(queryset, request)
        serializer = UserAdminSerializer(paginated_queryset, many=True, context={'request': request})
